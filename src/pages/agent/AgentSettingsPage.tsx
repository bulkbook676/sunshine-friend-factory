...
import { ArrowLeft, User, Lock, Building2, ChevronRight, LogOut, Send, Mic, Square, Play, Pause, Eye, EyeOff, X, Swords, Check, FileText, Download, ClipboardCheck } from "lucide-react";
    ...
    return (
      <div className="app-shell bg-background">
        <div className="page-content px-4 pt-4">
          ...
          <h1 className="text-xl font-bold text-foreground mb-1">Challenge Another Agent</h1>
  ...
  return (
    <div className="app-shell bg-background">
      <div className="page-content px-4 pt-4">
        ...
        <ProfileSummaryCard
          name={userName || "Chidi Okonkwo"}
          role="Agent"
          business={businessName || "Mama Nkechi Provisions"}
          bio="Top-performing sales agent — 78% consistency, 1,247 sales recorded."
          following={124}
          totalSales="₦247K"
        />
        ...
        <div className="space-y-1">
          {[
            ...
            { icon: Lock, label: "Change PIN", desc: "Update your 4-digit login PIN", action: () => navigate("/agent/settings/change-pin") },
            { icon: Building2, label: "Linked Business", desc: businessName || "Mama Nkechi Provisions", action: () => setActiveSection("linked") },
            { icon: Swords, label: "Challenge Another Agent", desc: "Compete with another agent", action: () => setActiveSection("challenge") },
            { icon: ClipboardCheck, label: "Stock Count", desc: "Submit physical inventory count", action: () => navigate("/agent/stock-count") },
          ].map((item, i) => (
            <button
              key={i}
              onClick={item.action}

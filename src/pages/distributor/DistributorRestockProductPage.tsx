  ...
  <div>
    ...
    <div className="relative">
      <select
        value={value}
        onChange={(e) => {
          if (e.target.value === "__add__" && onAddCustom) { onAddCustom(); return; }
...
const DistributorRestockProductPage = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  const { products, restockProduct } = useDistributor();
  ...
  const update = <K extends keyof RestockDraft>(k: K, v: RestockDraft[K]) => {
    setForm((p) => ({ ...p, [k]: v }));
  ...
  const oldIdeal = (product.costPrice ?? 0) * 1.3 || product.sellingPrice * 1.1;
  ...
  const submit = () => {
    if (!validate()) {
      toast.error("Please fix the errors in required fields");
      return;
    }
    ...
    <div className="app-shell dark bg-background">
      <div className="page-content px-4 pt-4 pb-6">
        ...
        <div className="space-y-4">
          ...
          <TextField label="Product Name" placeholder="" value={product.name} onChange={() => {}} readOnly />

          <div className="grid grid-cols-2 gap-3">
            <SelectField label="How you buy it" value={form.buyingUnit} onChange={(v) => update("buyingUnit", v)} options={allUnits} onAddCustom={() => promptAddCustomUnit("buyingUnit")} error={errors.buyingUnit} />
            <SelectField label="How you sell it" value={form.sellingUnit} onChange={(v) => update("sellingUnit", v)} options={allUnits} onAddCustom={() => promptAddCustomUnit("sellingUnit")} error={errors.sellingUnit} />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <TextField label={`How many ${buyingUnitLabel.toLowerCase()}${form.buyingUnit ? "s" : ""} did you order?`} placeholder="e.g. 10" value={form.buyingUnitsOrdered} onChange={(v) => update("buyingUnitsOrdered", v)} type="number" error={errors.buyingUnitsOrdered} />
            <TextField label={`How many ${sellingUnitLabel.toLowerCase()}${form.sellingUnit ? "s" : ""} in one ${buyingUnitLabel.toLowerCase()}?`} placeholder="e.g. 40" value={form.sellingUnitsPerBuying} onChange={(v) => update("sellingUnitsPerBuying", v)} type="number" error={errors.sellingUnitsPerBuying} />
          </div>
          ...
          <p className="text-xs text-muted-foreground">
            ...
            <span className="text-foreground font-semibold">{newTotal.toLocaleString()} units</span>
          </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-success/5 border border-success/20 rounded-lg p-4">
                ...
                {form.applyPriceToCurrent ? (
                  ...
                  <>
                    <p className="text-[11px] text-foreground leading-snug">Current stock: {onShelf} × {fmt(oldIdeal)} = {fmt(onShelf * oldIdeal)}</p>
                    <p className="text-[11px] text-foreground leading-snug mt-1">New stock: {Math.round(calc.addingStock)} × {fmt(newIdeal)} = {fmt(calc.addingStock * newIdeal)}</p>
                    <p className="text-lg font-bold text-success mt-2">{fmt(onShelf * oldIdeal + calc.addingStock * newIdeal)}</p>
              ...
              <div className="bg-primary/5 border border-primary/20 rounded-lg p-4">
                ...
                {form.applyPriceToCurrent ? (
                  ...
                  <>
                    <p className="text-[11px] text-foreground leading-snug">Current stock: {onShelf} × {fmt(oldActual)} = {fmt(onShelf * oldActual)}</p>
                    <p className="text-[11px] text-foreground leading-snug mt-1">New stock: {Math.round(calc.addingStock)} × {fmt(newActual)} = {fmt(calc.addingStock * newActual)}</p>
                    <p className="text-lg font-bold text-primary mt-2">{fmt(onShelf * oldActual + calc.addingStock * newActual)}</p>
                    <p className="text-[10px] text-muted-foreground mt-1">Prices split — current at old, new at new</p>
                  </>
                )}
              </div>
          ...
          <div className="border-t border-border pt-4 mt-2">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-4">Distributor Settings</p>
          </div>

          <TextField
            label="Free shipping on orders above (optional)"
...
export default DistributorRestockProductPage;